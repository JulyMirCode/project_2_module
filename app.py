...
app = FastAPI()
templates = Jinja2Templates(directory='templates') #объект шаблонов
...
async def upload_images(file: UploadFile = File(...)):
    ...
    if is_allowed_file(my_file):
        print('Верное расширения файла')
    else:
        print('Неверное расширение файла')
        raise HTTPException(status_code=400, detail='Неподдерживаемый файл')
    ...
    return PlainTextResponse(f'запрос отработал и получили {file.filename}')
